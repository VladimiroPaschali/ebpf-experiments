...
#include "cms_meglio.skel.h"


int if_index;
struct cms_meglio_bpf* cms_meglio;

void sig_handler(int sig) {
	bpf_xdp_detach(if_index, 0, NULL);
	cms_meglio_bpf__destroy(cms_meglio);
	exit(0);
}
...
int main(int argc, char **argv) {
	...
	cms_meglio = cms_meglio_bpf__open_and_load();
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(cms_meglio->progs.cms_meglio), 0, NULL);
	if (err) {
