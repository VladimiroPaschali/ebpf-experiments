...
int if_index;
struct cms_kfunc_bpf* cms;

void sig_handler(int sig) {
	bpf_xdp_detach(if_index, 0, NULL);
	cms_kfunc_bpf__destroy(cms);
	exit(0);
}
...
int main(int argc, char **argv) {
	...
	cms = cms_kfunc_bpf__open_and_load();
	err = cms_kfunc_bpf__attach(cms);
	if (err) {
	...
	err = bpf_xdp_attach(if_index, bpf_program__fd(cms->progs.cms_kfunc), 0, NULL);
	if (err) {
		fprintf(stderr, "Failed to attach BPF program\n");
		return 1;
	}
