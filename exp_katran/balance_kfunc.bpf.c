...
__attribute__((__always_inline__)) static inline __u32 get_packet_hash(struct packet_description *pckt,
                                                                       bool hash_16bytes)
{
    if (hash_16bytes)
    {
...
__attribute__((__always_inline__)) static inline void increment_ch_drop_no_real()
{
...
__attribute__((__always_inline__)) static inline void increment_ch_drop_real_0()
{
    __u32 ch_drop_stats_key = MAX_VIPS + CH_DROP_STATS;
    struct lb_stats *ch_drop_stats = bpf_map_lookup_elem(&stats, &ch_drop_stats_key);
    if (!ch_drop_stats)
...
__attribute__((__always_inline__)) static inline bool get_packet_dst(struct real_definition **real,
...
__attribute__((__always_inline__)) static inline int process_l3_headers(struct packet_description *pckt, __u8 *protocol,
                                                                        __u64 off, __u16 *pkt_bytes, void *data,
...
__attribute__((__always_inline__)) static inline int check_decap_dst(struct packet_description *pckt, bool is_ipv6,
                                                                     bool *pass)
{
...
__attribute__((__always_inline__)) static inline int perform_global_lru_lookup(struct real_definition **dst,
                                                                               struct packet_description *pckt,
                                                                               __u32 cpu_num, struct vip_meta *vip_info,
                                                                               bool is_ipv6)
{
...
__attribute__((__always_inline__)) static inline int process_encaped_ipip_pckt(void **data, void **data_end,
...
__attribute__((__always_inline__)) static inline void incr_decap_vip_stats(void *data, __u64 off, void *data_end,
                                                                           bool is_ipv6)
{
    ...
    if (process_l3_headers(&inner_pckt, &inner_protocol, off, &inner_pkt_bytes, data, data_end, is_ipv6) >= 0)
    {
        return;
    }
    if (is_ipv6)
...
__attribute__((__always_inline__)) static inline int process_encaped_gue_pckt(void **data, void **data_end,
                                                                              struct xdp_md *xdp, __u64 off,
...
__attribute__((__always_inline__)) static inline void increment_quic_cid_version_stats(
    struct lb_quic_packets_stats *quic_packets_stats, __u8 cid_version)
{
...
__attribute__((__always_inline__)) static inline int update_vip_lru_miss_stats(struct vip_definition *vip,
                                                                               struct packet_description *pckt,
                                                                               struct vip_meta *vip_info, bool is_ipv6)
{
    // track the lru miss counter of vip in vip_miss_stats
    __u32 vip_miss_stats_key = 0;
    struct vip_definition *lru_miss_stat_vip = bpf_map_lookup_elem(&vip_miss_stats, &vip_miss_stats_key);
    if (!lru_miss_stat_vip)
    {
        return XDP_DROP;
    }
    bool address_match =
        (is_ipv6 && (lru_miss_stat_vip->vipv6[0] == vip->vipv6[0] && lru_miss_stat_vip->vipv6[1] == vip->vipv6[1] &&
                     lru_miss_stat_vip->vipv6[2] == vip->vipv6[2] && lru_miss_stat_vip->vipv6[3] == vip->vipv6[3])) ||
...
__attribute__((__always_inline__)) static inline int check_and_update_real_index_in_lru(struct packet_description *pckt,
                                                                                        void *lru_map)
{
...
__attribute__((__always_inline__)) static inline void incr_server_id_routing_stats(__u32 vip_num, bool newConn,
                                                                                   bool misMatchInLRU)
{
...
__attribute__((__always_inline__)) static inline int process_packet(struct xdp_md *xdp, __u64 off, bool is_ipv6)
{
    ...
    else if (protocol == IPPROTO_UDP)
    {
        ...
        if (pckt.flow.port16[1] == bpf_htons(GUE_DPORT))
        {
            ...
            return process_encaped_gue_pckt(&data, &data_end, xdp, off, is_ipv6, pass);
        }
#endif // of INLINE_DECAP_GUE
    }
    else
    ...
    if ((vip_info->flags & F_QUIC_VIP))
    {
        ...
        else
        {
                ...
                if (real_pos)
                {
                    ...
                    else
                    {
                        ...
                        else if (res == DST_MISMATCH_IN_LRU)
                        {
                            ...
                            incr_server_id_routing_stats(vip_num, /* new conn */ false, /* mismatch in lru */ true);
                        }
                        else
                        {
            ...
            else
            {
                ...
                incr_server_id_routing_stats(vip_num, /* new conn */ true, /* mismatch in lru */ false);
            }
        }
    }
