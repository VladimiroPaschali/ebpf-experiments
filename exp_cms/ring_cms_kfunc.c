...
static void sig_handler(int sig)
{
	...
	ring_cms_kfunc_bpf__destroy(cms);
	exit(0);
}
...
int main( int argc, char **argv) {
	...
	cms = ring_cms_kfunc_bpf__open_and_load();
	rb = NULL;
	int err;
	...
	/* Attach tracepoint */
	//err = ring_cms_kfunc_bpf__attach(cms);
	//if (err) {
	...
	err = bpf_xdp_attach(if_index,  bpf_program__fd(cms->progs.ring_cms_kfunc), 0, NULL);
	if (err) {
