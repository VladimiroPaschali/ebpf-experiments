...
#include "fw_kfunc.skel.h"

int if_index;
struct fw_kfunc_bpf *fw;

void sig_handler(int sig)
{

    bpf_xdp_detach(if_index, 0, NULL);
    fw_kfunc_bpf__destroy(fw);
    exit(0);
}
...
int main(int argc, char **argv)
{
    ...
    fw = fw_kfunc_bpf__open_and_load();
    ...
    err = bpf_xdp_attach(if_index, bpf_program__fd(fw->progs.fw_kfunc), 0, NULL);
    if (err)
    {
