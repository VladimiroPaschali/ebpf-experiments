...
static int change_event(__u64 reg, __u64 event)
{
    ...
    data.reg_counter = reg;

    int fd = 0;
    int zero = 0;
    fprintf(stdout, "[%s]: Changing event %llu\n", INFO, event);
    fd = find_data_map(); // it is possible to call this one time and save the fd
    if (fd < 0)
    {
    ...
    if (err)
    {
        fprintf(stderr, "[%s]: during updating event\n", ERR);
        return -1;
    }
...
int main(int arg, char **argv)
{
    ...
    if (strlen(func_name) > MAX_PROG_FULL_NAME)
    {
        fprintf(stderr, "[%s]: Program name too long: %s\n", ERR, func_name);
        return 1;
    }
